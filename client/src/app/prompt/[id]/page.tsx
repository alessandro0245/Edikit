...
const ASSET_SLOTS: AssetSlotConfig[] = [
  ...
  { type: "media",      label: "Video Media",     hint: "Any image or video overlay", icon: <ImagePlus className="w-4 h-4" /> },
];
...
// ─── Asset Uploader ───────────────────────────────────────────────────────────
function AssetUploader({ assets, setAssets }: { 
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-1">
        <h3 className="text-sm font-semibold text-foreground tracking-tight">Assets (Optional)</h3>
        <p className="text-xs text-muted-foreground">Upload images or videos to include in your video.</p>
      </div>
...
// ─── Prompt Step ──────────────────────────────────────────────────────────────
function PromptStep({
  prompt, setPrompt, assets, setAssets, settings, setSettings,
  settingsOpen, setSettingsOpen, isLoggedIn, authLoading,
  isSubmitting, canRender, changeCount, onSubmit,
}: {
  prompt: string;
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-8">
      {/* Header */}
      <div>
        <h2 className="text-xl font-bold text-foreground tracking-tight">Create Video</h2>
        <p className="text-sm text-muted-foreground mt-1">Describe your video and optionally add assets.</p>
      </div>
...
export default function PromptPage() {
  const {
    ...
    isSubmitting, canRender, handleSubmit, handleReset, handleDownload, 
