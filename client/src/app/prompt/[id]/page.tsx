...
import {
  ArrowRight, Loader2, CheckCircle2, Menu, X,
  AlertTriangle, RefreshCw, Film, SlidersHorizontal,
  ImagePlus, Layers, Stamp, Upload, Sparkles, ChevronRight,
} from "lucide-react";
import { usePromptLogic } from "./usePromptLogic";
import VideoPlayer from "@/components/Video/VideoPlayer";
import VideoDownloadButton from "@/components/Video/VideoDownloadButton";
import { VideoSettingsModal, countChanges } from "@/components/VideoSettingsModal/VideoSettingsModal";
    ...
    <div className="space-y-7">
      ...
      <div className="space-y-1">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-[10px] font-mono text-muted-foreground uppercase tracking-[0.2em]">Step 1 of 2</span>
        </div>
        <h2 className="text-xl font-bold text-foreground tracking-tight">Brand Assets</h2>
              ...
              {urls.length > 0 ? (
                ...
                >
                  ...
                  <div className="w-10 h-10 rounded-lg bg-muted/40 border border-border/40 flex items-center justify-center text-muted-foreground group-hover:text-foreground group-hover:border-border transition-colors shrink-0">
                    {isActive ? <Loader2 className="w-4 h-4 animate-spin text-primary" /> : slot.icon}
                  </div>
                  {/* Text */}
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-foreground">{slot.label}</p>
                    <p className="text-[11px] text-muted-foreground/60 font-mono mt-0.5">{slot.hint}</p>
                  </div>
                  {/* Arrow */}
      ...
      <div className="flex gap-2.5 pt-1">
        ...
        >
          ...
          {uploadCount > 0 ? (
            <><Sparkles className="w-4 h-4" />Continue with {uploadCount} asset{uploadCount > 1 ? "s" : ""}</>
          ) : (
...
// ─── Step indicator ───────────────────────────────────────────────────────────
function StepIndicator({ steps, progress, progressStep }: {
      ...
      {steps.map((step, i) => {
        ...
        return (
          <div key={i} className="flex items-center gap-3">
            <div className={`shrink-0 w-6 h-6 rounded-full flex items-center justify-center transition-all duration-500
              ${isFailed   ? "bg-red-500/20 border border-red-500/40"
              : isComplete ? "bg-primary border border-primary"
...
function ProgressBar({ progress, isError }: { progress: number; isError: boolean }) {
  return (
    ...
    <div className="w-full max-w-4xl aspect-video relative overflow-hidden rounded-xl border border-border/50 bg-[#080808]">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center gap-5">
        ...
        <div className="text-center space-y-1.5">
          <p className="text-sm font-medium text-muted-foreground">Your video will appear here</p>
          <p className="text-xs text-muted-foreground/50">Fill in the prompt and hit Generate</p>
        </div>
        {[["top-4 left-4","border-t border-l"],["top-4 right-4","border-t border-r"],
          ...
          <div key={i} className={`absolute ${pos} w-5 h-5 ${border} border-border/30`} />
        ))}
      </div>
    </div>
  );
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center gap-6">
        <div className="relative w-20 h-20">
          <svg className="w-20 h-20 -rotate-90" viewBox="0 0 80 80">
            <circle cx="40" cy="40" r="34" fill="none" stroke="hsl(var(--muted))" strokeWidth="3" />
            <circle cx="40" cy="40" r="34" fill="none" stroke="hsl(var(--primary))" strokeWidth="3"
              strokeLinecap="round"
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="text-sm font-mono font-bold text-primary tabular-nums">{Math.round(progress)}%</span>
          </div>
        </div>
        <p className="text-sm text-muted-foreground animate-pulse">Rendering your video...</p>
      </div>
    </div>
  );
}
    ...
    <div className="w-full max-w-4xl aspect-video relative overflow-hidden rounded-xl border border-red-500/20 bg-[#0d0505]">
      <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 px-8 text-center">
        ...
        <div className="space-y-1.5">
          ...
          <p className="text-xs text-muted-foreground max-w-sm">{message ?? "An error occurred."}</p>
        </div>
      </div>
    </div>
  );
...
// ─── Prompt Step ──────────────────────────────────────────────────────────────
function PromptStep({
  prompt, setPrompt, assets, settings, setSettings,
  settingsOpen, setSettingsOpen, isLoggedIn, authLoading,
  isSubmitting, canRender, changeCount, onSubmit, onBack, 
}: {
  prompt: string;
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-5">
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          ...
          <div className="flex items-center gap-1.5">
            <div className="w-1 h-5 bg-primary rounded-full" />
            <span className="text-[10px] font-mono text-muted-foreground uppercase tracking-[0.2em]">Step 2 of 2</span>
          </div>
        </div>
        <h2 className="text-xl font-bold text-foreground tracking-tight">Your Prompt</h2>
      </div>
        ...
        <div className="flex flex-wrap gap-1.5">
          ...
          {assets.bgImageUrl && (
            <div className="flex items-center gap-1.5 px-2.5 py-1 bg-primary/8 border border-primary/20 rounded-lg">
              ...
              <span className="text-[11px] font-mono text-primary">Background</span>
            </div>
          )}
          {assets.watermarkUrl && (
            <div className="flex items-center gap-1.5 px-2.5 py-1 bg-primary/8 border border-primary/20 rounded-lg">
      ...
      <div className="space-y-2">
        <label htmlFor="prompt" className="text-xs font-semibold text-foreground uppercase tracking-widest">
          Describe Your Video
        </label>
        <div className="relative">
          ...
          />
          <span className={`absolute bottom-3 right-3.5 text-[10px] font-mono tabular-nums transition-colors ${prompt.length > 450 ? "text-red-400" : "text-muted-foreground/40"}`}>
            {prompt.length}/500
          </span>
        </div>
        {prompt.length > 0 && prompt.length < 10 && (
          <p className="text-[11px] text-muted-foreground">{10 - prompt.length} more characters needed</p>
        )}
      </div>
      ...
      {isLoggedIn && canRender === false && (
        <div className="flex items-start gap-2.5 p-3.5 bg-red-500/5 border border-red-500/20 rounded-xl">
          ...
          <p className="text-xs text-red-400 leading-relaxed">
            Not enough credits.{" "}
            <a href="/pricing" className="underline underline-offset-2 font-medium">Get more →</a>
          </p>
        </div>
      )}
      ...
      <div className="flex gap-2.5">
        ...
        <button
          type="submit"
          disabled={prompt.trim().length < 10 || authLoading || !isLoggedIn || isSubmitting || canRender === false}
        ...
        >
          <div className="absolute inset-0 bg-linear-to-r from-transparent via-white/10 to-transparent translate-x-[-200%] group-hover:translate-x-[200%] transition-transform duration-700" />
          {authLoading   ? <><Loader2 className="w-4 h-4 animate-spin" />Loading...</>
          : !isLoggedIn  ? <>Login to Generate<ArrowRight className="w-4 h-4" /></>
          : isSubmitting ? <><Loader2 className="w-4 h-4 animate-spin" />Starting...</>
          : <><Sparkles className="w-4 h-4" />Generate<ArrowRight className="w-4 h-4 group-hover:translate-x-0.5 transition-transform" /></>}
        </button>
      </div>
    </form>
  );
...
export default function PromptPage() {
  const {
    categoryId, pageStep, assets, handleAssetsComplete,
    prompt, setPrompt, settings, setSettings,
  ...
  const handleResetAndScroll = () => { handleReset(); };
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col lg:flex-row">
      ...
      <div className="lg:hidden sticky top-0 z-50 bg-card/95 backdrop-blur border-b border-border px-4 py-3.5 flex items-center justify-between">
        <div>
          <h1 className="text-sm font-semibold text-foreground tracking-tight">Create Video</h1>
          <p className="text-[11px] text-muted-foreground mt-0.5 capitalize">{categoryId.replace(/-/g, " ")}</p>
        </div>
        <button onClick={() => setSidebarOpen(!sidebarOpen)} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-muted transition-colors">
          {sidebarOpen ? <X className="w-4 h-4" /> : <Menu className="w-4 h-4" />}
        </button>
      </div>

      {/* Sidebar */}
      <aside className={`${sidebarOpen ? "flex" : "hidden"} lg:flex w-full lg:w-100 xl:w-110 flex-col bg-card border-b lg:border-b-0 lg:border-r border-border lg:h-screen lg:sticky lg:top-0`}>

        {/* Desktop header */}
        <div className="hidden lg:flex flex-col px-8 pt-10 pb-7 border-b border-border gap-1">
          <h1 className="text-2xl font-bold text-foreground tracking-tight">Create Video</h1>
          <p className="text-sm text-muted-foreground capitalize">{categoryId.replace(/-/g, " ")}</p>
        </div>
            ...
            <div className="space-y-6">
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-xs font-mono tabular-nums ${isError ? "text-red-400" : isComplete ? "text-primary" : "text-muted-foreground"}`}>
              ...
              <StepIndicator steps={steps} progress={progress} progressStep={progressStep} />
              ...
              {isError && errorMessage && (
                <div className="p-4 bg-red-500/5 border border-red-500/20 rounded-xl space-y-1">
                  ...
                  <p className="text-[11px] text-muted-foreground">Credits have been refunded.</p>
                </div>
              )}

              <div className="p-4 bg-background/60 border border-border/60 rounded-xl">
                <p className="text-[10px] font-mono uppercase tracking-widest text-muted-foreground mb-1.5">Prompt</p>
                <p className="text-xs text-foreground/80 leading-relaxed line-clamp-3">{prompt}</p>
              </div>
              ...
              {(isComplete || isError) && (
                ...
                >
                  {isError ? <><RefreshCw className="w-3.5 h-3.5" />Try Again</> : "Create Another Video"}
                </button>
              )}
              ...
              {isComplete && outputUrl && (
                <VideoDownloadButton
                  ...
                  tooltip={downloadError ? "Link expired. Please refresh." : "Download your video"}
        ...
        <div className="px-8 py-4 border-t border-border bg-muted/20">
          <p className="text-[11px] text-muted-foreground text-center font-mono">
            {isIdle && pageStep === "assets" ? "All assets optional — skip to continue"
            : isIdle      ? "5 credits per generation"
            : isError     ? "Credits refunded on failure"
            : isComplete  ? "✓ Video ready to download"
      ...
      {/* Main canvas */}
      <main className="flex-1 flex flex-col items-center justify-start p-6 lg:p-14 gap-6 min-h-[90vh] lg:min-h-screen">
        {isComplete && outputUrl ? (
