...
async generateMatchCut(dto: GenerateMatchCutDto, userId: string) {
  ...
  // Run scene generation and render asynchronously in the background so the HTTP request
  // returns immediately (avoiding Vercel / proxy 502 Bad Gateway timeouts)
  this.triggerAsyncMatchCut(
    renderJob.id,
    userId,
    trimmedWord,
    durationInSeconds,
    ...
    this.logger.error(
      `Async matchcut background process ${renderJob.id} failed:`,
      (err as Error).stack,
    );
  });
...
private async triggerAsyncMatchCut(
  jobId: string,
  userId: string,
  trimmedWord: string,
  ...
  try {
    // 1. Generate scenes via LLM (or fallback) in background
    const scenes: MatchCutScene[] =
      await this.promptService.generateMatchCutScenes(
        trimmedWord,
        durationInSeconds,
      );
    ...
    // 2. Persist generated scenes into renderJob
    await this.prisma.renderJob.update({
      where: { id: jobId },
      data: {
        aiConfig: fullConfig as any,
      },
    });

    // 3. Trigger rendering
    await this.triggerAsyncRender(jobId, userId, fullConfig);
  } catch (error) {
    this.logger.error(
      `Async matchcut processing failed for job ${jobId}:`,
    ...
    await this.prisma.renderJob.update({
      ...
      data: {
        status: 'FAILED',
        error: (error as Error).message || 'Failed to generate match cut',
      },
    });
  }
}

// buildMatchCutScenes removed — scenes are now generated by PromptService.generateMatchCutScenes()
